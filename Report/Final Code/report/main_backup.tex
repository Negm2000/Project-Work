...
The primary objective is to reliably detect the presence or absence of cable connections under real industrial conditions, with emphasis on minimizing false negatives (undetected defects represent higher operational risk than triggering additional human inspection). The system must be deployable on standard industrial PCs without GPU infrastructure and scalable to different connector types without complete redesign.
...
\textbf{Illumination normalization.} A two-stage approach applies CLAHE (Contrast Limited Adaptive Histogram Equalization, clipLimit=2.5, tileGrid=8$\times$8) to the L channel in LAB colour space, followed by gray-world white balance. This reduced morning-to-afternoon intensity variation by 68\%, from 1.73 to 0.54 grayscale units.
...
% =============================================================================
% CHAPTER 4 -- EVOLUTION OF DETECTION APPROACHES (EXPANDED)
% Replace the existing \section{Evolution of Detection Approaches} block
% with this content.
% =============================================================================

\section{Evolution of Detection Approaches}
...
\textbf{PatchCore anomaly detection.} PatchCore~\cite{roth2022patchcore} models only the appearance of normal (OK) data and flags deviations from that learned normality. Rather than operating on global embeddings, it extracts intermediate feature maps from ResNet18 (up to \texttt{layer2}), producing spatial feature tensors $\mathbf{F} \in \mathbb{R}^{128 \times H_f \times W_f}$. Flattening yields patch descriptors $\mathcal{P} = \{\mathbf{p}_1, \dots, \mathbf{p}_N\}$ with $\mathbf{p}_i \in \mathbb{R}^{128}$, each representing a small local region of the connector and capturing geometry, cable edges, shadows and cavity texture. The memory bank is constructed exclusively from OK patches: (1) extract and normalize each patch $\hat{\mathbf{p}} = \mathbf{p} / \|\mathbf{p}\|$; (2) apply coreset sampling to reduce millions of patches to a compact memory bank $M = \{\mathbf{m}_1, \dots, \mathbf{m}_K\}$ with $K \approx 50{,}000$. For a test image, each patch is compared to the nearest memory entry and the anomaly score takes the maximum deviation:
\begin{equation}
...
\textbf{Architecture.} EfficientAD employs two networks. The \emph{Teacher} is a frozen ResNet18 pretrained on ImageNet, serving as a fixed feature extractor. The \emph{Student} is a randomly initialised ResNet18 with identical architecture, trained to mimic the Teacher's feature maps on OK images only. At test time, the squared difference between Teacher and Student feature maps is averaged across channels to produce a low-resolution anomaly map; the maximum value of this map becomes the scalar anomaly score. An additional autoencoder branch targets ``logical'' anomalies where a part is visually correct but structurally wrong (e.g.\ a cable routed incorrectly). The published benchmarks report Image-Level AU-ROC of 96.0\% and Pixel-Level AU-PRO of 93.3\% on MVTec AD and LOCO datasets with approximately 4.5\,ms latency per image.
...
\textbf{Multi-step system (Phase 2b).} Building on Phase 2's insights, a multi-step pipeline was developed (led by Giovanni Passuello) that added a dedicated Occlusion Classifier in front of EfficientAD. The OcclusionCNN is a lightweight three-block convolutional network (Conv2d $\to$ BatchNorm $\to$ ReLU $\to$ MaxPool, expanding from 32 to 128 channels, followed by a fully connected head with 512 neurons and dropout of 0.5) that classifies each connector crop as OCCLUSION or VISIBLE. Only VISIBLE crops proceed to EfficientAD for OK/KO classification. This two-stage design achieved 95.13\% overall accuracy on the full 2{,}853-image dataset, with per-class accuracy of 98.16\% for OK, 84.00\% for KO and 81.03\% for OCCLUSION. The dominant error mode was OCCLUSION samples predicted as KO (56 cases, 11.5\% of occlusions), confirming that even with explicit occlusion filtering, partial occlusions visually resemble structural anomalies.
...
These findings motivated the shift from anomaly detection to synthetic negative classification described in Section~5, where the problem is reformulated as binary presence detection using inpainting-generated training data.

% CHAPTER 5 -- THE FINAL PIPELINE
% =============================================================================
\section{Final Pipeline: PCL Presence Detection with Synthetic Negatives}
...
The advantage of this formulation is fundamental: instead of requiring a collection of rare real KO samples (of which we have fewer than 5 per connector), we generate unlimited synthetic KO examples by programmatically removing the cable from real OK images. The classifier learns the visual signature of ``cable present'' versus ``cable absent'' using matched pairs derived from the same source image, eliminating domain gap issues that would arise from using generic data augmentation.
...
Because the model outputs a continuous $P_{\mathrm{KO}}$ value, we can analyse separation quality beyond binary accuracy. For each connector, histograms of $P_{\mathrm{KO}}$ for OK and KO samples are plotted with the calibrated threshold $\tau$ overlaid. Clean separation (no histogram overlap) indicates high confidence. Overlap indicates borderline cases that benefit from refined masks, improved camera stability or expanded training data.

The Grad-CAM~\cite{selvaraju2017gradcam} saliency maps computed on ResNet18's \texttt{layer4} confirm that the model attends to the mask-defined PCL region for both OK and KO predictions, validating that the learned features are semantically meaningful rather than artefactual.
...
These error modes guide practical production improvements: tightening mechanical alignment, refining mask polygons based on false positive heatmaps and expanding the OK training pool via operator-verified closed-loop data collection.
