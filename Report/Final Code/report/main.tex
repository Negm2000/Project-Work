...
This report presents the development of an automated visual inspection system for PCB connector assemblies in oven production line at Beko Europe. The final deliverable is a deployable software pipeline capable of detecting missing cable connections with \textbf{95.13\% overall accuracy} across nine connector types and 2{,}853 images.
...
This architecture supersedes earlier exploration of PatchCore and EfficientAD, both of which struggled with the three-class OK/KO/OCCLUSION problem. The final pipeline is lightweight, reproducible and maintainable for long-term factory deployment.
...
\textbf{Extreme class imbalance.} The dataset contains $2,853$ connector images, reflecting the real-world rarity of industrial defects. The distribution is highly skewed: only $25$ instances ($0.9\%$) are genuine KO cases, whereas $485$ ($17\%$) represent non-defective occlusions and $2,343$ ($82.1\%$) are OK. A classical classifier trained naively on this distribution without proper imbalance handling technique fell into the accuracy paradox, achieving $>99\%$ accuracy by consistently predicting the majority class while failing to identify a single critical defect as in a predictable way \cite{anand2020guide}.

\textbf{Visual ambiguity from occlusions.} Adjacent cables frequently obstruct the view of connectors, introducing visual patterns absent from the nominal training set. Because these occlusions obscure the component's expected features, a naive anomaly detector perceives them as distributional shifts similar to actual structural defects like missing components. This leads to the misclassification of benign occlusions as anomalies, resulting in unacceptably high false positive rates.
...
% =============================================================================
\section{Final Pipeline: Synthetic Negative Detection}

This section details the final training and inference pipeline, which addresses the inherent rarity of industrial defects by recasting the anomaly detection challenge as a \textbf{standard supervised learning task}. The central methodology involves the generation of \textbf{realistic synthetic negatives} through digital inpainting techniques applied within connector-specific masks. By simulating anomalous patterns via inpainting, the system can be trained to recognize structural deviations despite the \textbf{crucial requirement} of having access to very few genuine "KO" instances \cite{telea2004inpainting}.

\subsection{Problem Reframing and Motivation}

For each connector crop, the decision is reduced to a binary presence check: is the PCL (cable) present or absent in the expected region? This framing matches the actual manufacturing requirement.

The advantage of this formulation is fundamental to handle inherent challenge in industrial inspection by bypassing the requirement for rare genuine defect samples \cite{batzner2023efficientad}. By utilizing digital inpainting via the \textbf{Fast Marching Method (FMM)}, the pipeline programmatically propagates known image information inward to remove cables from nominal (OK) images, generating an unlimited corpus of synthetic negatives. The classifier learns to identify the structural signature of ``cable presence'' versus ``cable absence'' using matched pairs derived from identical source images. This strategy effectively bypasses the \textbf{accuracy paradox} common in skewed industrial distributions and ensures high-fidelity training by avoiding the distributional shifts and artifacts typically introduced by generic data augmentation.
...
\subsection{Threshold Calibration and the Max + Margin Heuristic}

Following the principle of separation of roles between learning and calibration, the decision threshold is determined post-training using only real nominal (OK) instances. This strategy addresses the \textbf{cold-start challenge} by ensuring the model's sensitivity is grounded in the true distribution of defect-free production data rather than synthetic approximations. 

After training, inference is performed on the validation set $\mathcal{V}_{\mathrm{OK}}$. For each image $i$, the model produces a logit $z^{(i)}$, which is mapped to a KO probability via the sigmoid function: $P_{\mathrm{KO}}^{(i)} = \sigma(z^{(i)})$. The decision threshold $\tau$ is then established by applying a 15\% safety margin to the maximum probability observed across all nominal samples:
\begin{equation}
    \tau = \max_{i \in \mathcal{V}_{\mathrm{OK}}} \left( P_{\mathrm{KO}}^{(i)} \right) \times (1 + \alpha), \quad \alpha = 0.15
\end{equation}

This \textbf{conservative heuristic} acts as an operational buffer, designed to absorb environmental noise, lighting fluctuations, and minor domain shifts inherent in industrial environments. By placing the threshold above the "worst-case" nominal sample, the system targets a zero False Positive Rate (FPR) on validation data. This method provides significant maintenance flexibility. The threshold is connector-specific and can be recalibrated to adapt to new setup conditions \textbf{without requiring a full retraining} of the underlying ResNet-18 feature extractor.
...
Validation AUC-ROC & TBD & Pending recomputation \\
Expected validation FPR & TBD & At calibrated threshold \\
...
\midrule
conn1 & TBD & TBD & TBD & 136$\times$104 \\
conn2 & TBD & TBD & TBD & 299$\times$118 \\
conn3 & TBD & TBD & TBD & 224$\times$131 \\
conn4 & TBD & TBD & TBD & 254$\times$150 \\
conn5 & TBD & TBD & TBD & 168$\times$232 \\
conn6 & TBD & TBD & TBD & 108$\times$99 \\
conn7 & TBD & TBD & TBD & 194$\times$116 \\
conn8 & TBD & TBD & TBD & 139$\times$122 \\
conn9 & TBD & TBD & TBD & 151$\times$120 \\
\bottomrule
\end{tabular}
\caption{Per-connector performance of the final deployed system. Metrics are pending recomputation with the final model configuration.}
\label{tab:per_connector}
\end{table}

The threshold variability across connectors reflects inherent differences in visual complexity, geometry, and lighting conditions. For each connector, the decision threshold $\tau$ is calibrated following the Max(OK) + 15\% margin method to ensure robustness against production variability. Final metrics, including ROC curves and separation margins, are subject to final validation on the recomputed model set.
...
In addition to model development, a complete software application was implemented to enable practical deployment of the proposed inspection pipeline in an industrial environment. The system integrates preprocessing, inference and visualization into a single operator-oriented tool. Even if this project was intended to be a "proof of a concept", the team went beyond with ready-to-use in real time solution.

\subsection{Architecture}

The software is implemented in Python and follows a modular pipeline:
...
Calibrating $\tau$ from real OK validation images with a safety margin provides a production-oriented control mechanism. If the plant wants fewer false positives (e.g.\ to handle increased factory lighting noise), $\tau$ can be increased by choosing a larger safety margin $\alpha$ (e.g.\ $\alpha=0.25$). If greater sensitivity is required for subtle cables, $\alpha$ can be lowered (e.g.\ $\alpha=0.10$). This change is a deployment parameter requiring no retraining, making it suitable for operational tuning by plant engineers without machine learning expertise.
